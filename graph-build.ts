// Agents Office — bake the Brain graph from a vault's wiki links (Beta).
// Reads every .md under the configured brain folder (office.config.json → brain, default ./brain),
// resolves [[links]] by note name, lays the connected notes out and writes src/braingraph.js —
// the demo/offline graph. The server rebuilds the same data live (see readVault / layoutGraph).
...
// nested vault COPIES (shipped template vaults) would duplicate MOC names — skip them
...
/** Walk a vault: notes (name → { group, path, text }) and raw [[links]]. */
...
/** Notes the agents write live: <vault>/Agents Office/<file>.md — read separately, they DO count. */
...
/** Lay out a vault's link graph → { notes, nodes, links, floor } (the shape src/braingraph.js exports). */
export async function layoutGraph(vault: string): Promise<GraphResult> {
  const { notes, raw } = readVault(vault);
  for (const n of readOfficeNotes(vault)) { // live-written notes join the graph under their own group
  ...
  // layout: d3-force with the same settings as the approved mockup (a spread, even mesh);
  // falls back to a small Fruchterman–Reingold pass if d3-force is not installed
  ...
  // the FLOOR drawing uses the mock's recipe exactly: the 90 most linked notes, laid out on their own
  // (link 20 / .5, charge -30, collide 3, 300 ticks) so the mesh is compact and dense
...
/** Write src/braingraph.js from the configured brain (the offline/demo graph). */
