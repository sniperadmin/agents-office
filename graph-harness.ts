// Agents Office — LangGraph-Style State Graph Multi-Agent Harness.
...
/**
 * Plan Node: Formulates execution steps and strategy
 */
...
/**
 * Execute Node: Runs the agent work to produce/update the deliverable
 */
...
/**
 * QA Evaluator Node: Reflective node checking quality and assigning score & feedback
 */
export async function qaEvalNode(state: GraphState, helpers: GraphHelpers): Promise<GraphState> {
  ...
  try {
    ...
    // Default fallback QA evaluation
...
/**
 * Refine Node: Prepares feedback state for re-execution loop
 */
...
/**
 * State Graph Engine: Executes nodes and handles conditional edge transitions
 */
    ...
    switch (state.status) {
      ...
      case 'reviewing':
        ...
        if (state.qaResult?.pass) {
          ...
          state.status = 'completed'; // Accept best effort after max retries
        } else {
