// Agents Office V3.5 — routine schedules. Shared by the page (src/tasks.ts) and the server
// (routines.ts): plain words → a schedule, a schedule → the next time it is due, and a
...
const SHORT: string[] = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
const WORD_TIMES: Record<string, string> = { noon: '12:00', midday: '12:00', lunchtime: '12:30', midnight: '00:00', morning: '08:00', mornings: '08:00', afternoon: '14:00', afternoons: '14:00', evening: '17:00', evenings: '17:00', night: '20:00', nights: '20:00' };
const pad = (n: number): string => String(n).padStart(2, '0');
const hhmm = (h: number, m = 0): string => `${pad(h)}:${pad(m)}`;

function clock(h: number | string, m: number | string = 0, ap?: string | null): string | null {
  let numH = +h; let numM = +(m || 0);
  if (ap === 'pm' && numH < 12) numH += 12;
  if (ap === 'am' && numH === 12) numH = 0;
  if (numH > 23 || numM > 59) return null;
  return hhmm(numH, numM);
}

const T_RE = /\b(?:at\s+)?(\d{1,2})(?::(\d{2}))\s*(am|pm|a\.m\.|p\.m\.)?\b|\bat\s+(\d{1,2})\s*(am|pm|a\.m\.|p\.m\.)?\b|\b(\d{1,2})\s*(am|pm|a\.m\.|p\.m\.)\b/i;
function findTime(s: string): { at: string; span: [number, number]; guessed: boolean; word?: string } | null {
  const m = T_RE.exec(s);
  if (m) {
...
const cut = (s: string, span: [number, number]): string => (s.slice(0, span[0]) + ' ' + s.slice(span[1]));
...
export function parseWhen(input?: string): ParseResult | null {
  const src = String(input || '');
  let s = src, m: RegExpExecArray | null;
  // every N minutes
  ...
  if ((m = /\b(?:hourly|every\s+(\d+\s+)?hours?|each\s+hour|once\s+an\s+hour)\b/i.exec(s))) {
    const when: Schedule = { kind: 'hourly', every: Math.max(1, +(m[1] || 1)) };
    ...
    if (w) {
      ...
      if (a && b) {
        if (!w[3] && !w[6] && +w[1] < 8 && +w[1] < +w[4]) a = clock(+w[1] + 12, w[2]);
        if (b < a && !w[6]) b = clock((+w[4] % 12) + 12, w[5]);
...
export function fromPicker(cadence: string, at?: string): Schedule {
...
export function describe(when?: Schedule): string {
  if (!when) return '';
  const at = when.at ? ' · ' + when.at : '';
  switch (when.kind) {
    case 'minutes': return `every ${when.every} min`;
    case 'hourly': return (when.every && when.every > 1 ? `every ${when.every} hours` : 'every hour') + (when.from ? ` ${when.from}–${when.to}` : '') + (when.weekdaysOnly ? ' · weekdays' : '');
...
export function valid(when?: Schedule): boolean {
  if (!when || typeof when !== 'object') return false;
  const t = (s?: string | null) => /^\d{2}:\d{2}$/.test(s || '');
  if (when.kind === 'minutes') return Number.isInteger(when.every) && (when.every || 0) >= 1;
  if (when.kind === 'hourly') return Number.isInteger(when.every) && (when.every || 0) >= 1 && (!when.from || (t(when.from) && t(when.to) && (when.from! < when.to!)));
...
export function nextRun(when: Schedule, from = Date.now()): number | null {
  if (!valid(when)) return null;
  const f = new Date(from);
  if (when.kind === 'minutes') { const step = (when.every || 1) * 60000; return Math.floor(from / step) * step + step; }
  if (when.kind === 'hourly') {
    const start = when.from ? mins(when.from) : 0, end = when.to ? mins(when.to) : 24 * 60, step = (when.every || 1) * 60;
    const d = new Date(f); d.setSeconds(0, 0); d.setMinutes(0); d.setHours(d.getHours() + 1);
...
export function untilText(ts?: number | null, now = Date.now()): string {
  if (!ts) return '—';
  const ms = ts - now;
  if (ms <= 0) return 'now';
